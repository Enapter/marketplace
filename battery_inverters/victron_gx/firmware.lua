...
function send_properties()
  ...
  if err then
    ...
    enapter.send_properties({
      ...
      vebus_unit_id = values[VEBUS_UNIT_ID],
      mode2_unit_id = values[MODE2_UNIT_ID]
    })
  end
end
...
function connect_victron_gx()
  ...
  if victron_gx then
    if victron_gx.ip_address ~= ip_address or victron_gx.vebus_unit_id ~= vebus_unit_id or
      victron_gx.mode2_unit_id ~= mode2_unit_id then

      victron_gx.ip_address = ip_address
      victron_gx.vebus_unit_id = vebus_unit_id
      victron_gx.mode2_unit_id = mode2_unit_id
    end
...
function VictronGX.new()
  ...
  assert(type(mode2_unit_id) == 'number', 'mode2_unit_id (arg #3) must be number, given: '..inspect(mode2_unit_id))
