...
function send_properties()
  ...
  if err then
    enapter.log('ailed to get properties: ' .. err, 'error', true)
  else
    if not parse_properties(properties, results) then
      enapter.log('failed to parse properties', 'error', true)
    end
  end
...
function parse_metrics2(telemetry, data)
  ...
  telemetry.pv_voltage = sum / count
end
...
function parse_properties(properties, results)
  ...
  local device_type = try_get_device_type(results[1])
  if not device_type then
    enapter.log('no device type found', 'error', true)
    return false
  end

  fill_general_properties(properties, results[1], device_type)
  fill_battery_properties(properties, results[2], 98)
  fill_parallel_mode(properties, results[3])

  return true
end

function try_get_device_type(data)
  if not data then
    return
  end
  local device_types = {
  ...
  if not device_type or not string.find(device_type, 'hybrid') then
    enapter.log('skipping properties because device type is not supported: ' .. tostring(device_type), 'error')
    return nil
  end
  return device_type
end

function fill_general_properties(properties, data, device_type)
  if not data or not properties then
    return
  end
  ...
  properties.communication_protocol_version = tostring(data[3] >> 8) .. '.' .. tostring(data[3] & 0xFF)
  properties.serial_number = get_ascii_string(data, 4, 5)
  properties.firmware_version = string.format('%04X', data[15])
    .. '-'
    .. string.format('%04X', data[16])
    .. '-'
    .. string.format('%04X', data[12])

  local inverter_nameplate_capacity = ((data[22] << 16) + data[21]) / 10
...
function fill_battery_properties(properties, data, base)
  if not data or not properties then
    return
  end
  if data[98 - base + 1] == 0x0000 then
    properties.battery_type = 'lead_based'
  elseif data[98 - base + 1] == 0x0001 then
    properties.battery_type = 'lithium_based'
  end
  properties.battery_nameplate_capacity = data[102 - base + 1] -- Ah
end
