...
function reconnect_can()
  ...
  if not configuration.is_all_required_set('can') then
    return
  end

  local config, err = configuration.read('can')
  if err ~= nil then
...
function reconnect_relays()
  if power_relay and start_relay then
    return
  end

  if not configuration.is_all_required_set('relay') then
    return
  end
  ...
  local pr, err = relay.new(conn_cfg.power_relay_connection_uri)
  if err then
    enapter.log('failed to create power relay client: ' .. err, 'error')
    return
  end
  local sr, err = relay.new(conn_cfg.start_relay_connection_uri)
  if err then
...
function send_telemetry()
  if not conn_cfg then
    enapter.send_telemetry({ status = 'conn_error', conn_alerts = { 'not_configured' } })
    return
  end
  ...
  telemetry.fuel_cell_status = to_fuel_cell_status(telemetry.status)
  telemetry.energy_generated_lifetime = 0.0002777778 * telemetry.total_run_energy
  telemetry.run_hours_lifetime = telemetry.run_hours / 3600
...
function build_message_specs()
  ...
  message_specs = {
    ...
    telemetry = {
      ...
      {
        names = { 'output_power', 'output_voltage', 'output_current', 'h2_inlet_pressure' },
      ...
      {
        names = { 'outlet_temperature', 'inlet_temperature', 'output_voltage_setpoint', 'output_current_limit' },
...
function cmd_start(ctx)
  ...
  if not is_powered then
    ctx.error('cannot start a powered-off fuel cell, power it on first')
    return
  end
...
function cmd_power_off(ctx)
  ...
  if is_started then
    ctx.error('cannot power off a started fuel cell, stop it first')
  end
...
function parse_status(data)
  local bit_mask = string.sub(data, 1, 2)
  local to_iefc_status_map = {
    ['10'] = 'fault',
    ['20'] = 'standby',
    ['40'] = 'run',
    ['80'] = 'inactive',
  }
  return to_iefc_status_map[bit_mask]
end

function to_fuel_cell_status(st)
  local to_fuel_cell_status_map = {
    fault = 'fault',
    standby = 'standby',
    run = 'running',
    inactive = 'idle',
  }
  return to_fuel_cell_status_map[st]
end
