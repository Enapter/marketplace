...
GAS_WARN_THRESHOLD = 'gas_warning_threshold'
...
function main()
  ...
  config.init({
    ...
    [GAS_WARN_THRESHOLD] = { type = 'number', required = false },
  })
...
function get_alerts_and_status(telemetry)
  ...
  if err then
    ...
    local gas_warning_threshold = values[GAS_WARN_THRESHOLD]

    if telemetry.gas == true then
      table.insert(alerts, 'gas_critical')
      critical_present = true
    elseif gas_warning_threshold and telemetry.gas_density >= gas_warning_threshold then
      table.insert(alerts, 'gas_warning')
